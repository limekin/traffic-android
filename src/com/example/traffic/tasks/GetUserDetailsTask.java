...
public class GetUserDetailsTask extends AsyncTask<String, JSONObject, JSONObject> {
	
	private ProfileActivity sourceActivity;
	
	public GetUserDetailsTask(ProfileActivity activity) {
	...
	protected void onPostExecute(JSONObject result) {
		sourceActivity.setUserData(result);
	}
	
	protected JSONObject getUserDetails() throws IOException {
		String jsonResponse = "";
		JSONObject responseJson = new JSONObject();
		Log.i("HTTP", "Retrieving user details ...");
		
		try {
			String token = AppConfig.get_auth_token(this.sourceActivity);
			String url = AppConfig.API_BASE_URL + "/users/current";
			url += "?token=" + token;
